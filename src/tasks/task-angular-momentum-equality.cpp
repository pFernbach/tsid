...
namespace tsid
{
  namespace tasks
  {
    ...
    const Vector3 & TaskAMEquality::momentum_ref() const
    {
      return m_ref.vel;
    }

    const Vector3 & TaskAMEquality::dmomentum_ref() const
    {
      return m_ref.acc;
    }
                                                    ...
                                                    const Data & data)
    {
      ...
      double dt = 1e-3;
      const Vector & q_next = se3::integrate(m_robot.model(), q, dt*v);
      Data data_ccrba = data;
      se3::ccrba(m_robot.model(), data_ccrba, q_next, v);
